...
void MainWindow::ImageCallBackInner2(MV_FRAME_OUT* pstFrame, bool bAutoFree)
{
    ...
    stDisplayInfo.pData = pstFrame->pBufAddr;
    stDisplayInfo.nDataLen = pstFrame->stFrameInfo.nFrameLen;
    stDisplayInfo.nWidth = pstFrame->stFrameInfo.nWidth;
    stDisplayInfo.nHeight = pstFrame->stFrameInfo.nHeight;
...
QString MainWindow::GetMoudlePath()
{
    QString strCtiPath = QCoreApplication::applicationDirPath();
    Dl_info info;
    int rc = dladdr((void *)MV_CC_GetSDKVersion, &info);
    if (rc)
    {
        QString strDllPath = info.dli_fname;
        int nPos = strDllPath.lastIndexOf('/');
        strCtiPath = strDllPath.mid(0, nPos);
    }
    return strCtiPath;
}
...
void MainWindow::on_EnumInterface_clicked()
{
    ...
    QString strCtiPath = GetMoudlePath();
    QString FilePath = QFileDialog::getOpenFileName(this,QString::fromLocal8Bit("文件对话框！"), strCtiPath,
...
void MainWindow::on_EnumDevice_clicked()
{
	...
	if(ui->InterfacecomboBox->currentIndex() < 0 || ui->InterfacecomboBox->currentIndex() >=m_stIFList.nInterfaceNum )
	{
		 EnableControls(false);
		ShowErrorMsg(("No Device"), 0);
        return;
