...
def _create_or_resume_dataset(
    ...
    *,
    ...
    if cfg.resume and os.path.exists(cfg.dataset.root):
        dataset = LeRobotDataset(
            cfg.dataset.repo_id,
...
def record(cfg: HandheldRecordingConfig) -> LeRobotDataset:
    ...
    # 构建 dataset features
    features = build_dataset_features(cameras, tactiles, handheld_grippers, use_videos=cfg.dataset.video)
    cfg.resume = True  # 尝试 resume dataset
    ...
    print("键盘控制模式：按 r 开始录制，s 保存当前 episode，q 退出程序。")
    ...
    def on_press(key):
        nonlocal recording, episode_start_time, stop_listener
        try:
            if key.char == 'r' and not recording:
                ...
                print(f"Episode 保存成功，总计 {dataset.num_episodes} episodes")
            elif key.char == 'q':
                print("退出录制...")
                if recording:
        ...
        with VideoEncodingManager(dataset):
            ...
            with keyboard.Listener(on_press=on_press) as listener:
                while not stop_listener:
                    if recording:
                        frame = collect_dataset_frame(
                            cameras, tactiles, handheld_grippers,
                            max_read_age_ms=cfg.sensors.max_read_age_ms,
                            episode_start_time_s=episode_start_time,
                            task=cfg.dataset.single_task,
                        )
                        dataset.add_frame(frame)
                        if rerun_enabled:
                            _log_rerun_frame(
                                frame_index=dataset.episode_buffer["size"] - 1,
                                dataset_timestamp_s=time.perf_counter() - episode_start_time,
                                cameras=cameras,
                                tactiles=tactiles,
                                handheld_grippers=handheld_grippers,
                                frame=frame,
                            )
                    time.sleep(0.01)  # 控制采集间隔
    finally:
