...
class Panda {
 public:
  ...
  Vector7d getJointLimitsLower() const { return joint_limits_.lower; }
  Vector7d getJointLimitsUpper() const { return joint_limits_.upper; }
  bool moveToJointPositionConfigured(
      std::vector<Vector7d> waypoints, double speed_factor,
      double max_deviation, double planning_timeout,
  ...
  void startControllerGuarded(std::shared_ptr<TorqueController> controller);
  bool controlThreadActive() const { return control_thread_active_.load(std::memory_order_acquire); }
  void stopController();
 ...
 private:
  void _startController(std::shared_ptr<TorqueController> controller, bool allow_recovery = true);
  ...
  RobotJointLimits joint_limits_;
