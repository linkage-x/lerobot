...
void Panda::startControllerGuarded(std::shared_ptr<TorqueController> controller_ptr) {
  if (last_error_ || (current_controller_ && current_controller_->isRunning()))
    throw std::runtime_error("Guarded start refuses pending error or active controller");
  if (current_thread_.joinable()) current_thread_.join();
  _startController(controller_ptr, false);
  control_thread_active_.store(true, std::memory_order_release);
  try {
    current_thread_ = std::thread(
        std::bind(&Panda::_runController, this, _createTorqueCallback()));
  } catch (...) {
    control_thread_active_.store(false, std::memory_order_release);
    throw;
  }
}

void Panda::_startController(std::shared_ptr<TorqueController> controller_ptr, bool allow_recovery) {
  if (allow_recovery) {
    recover();
  } else {
    const auto state = robot_->readOnce();
    if (state.current_errors || state.robot_mode != franka::RobotMode::kIdle) {
      throw std::runtime_error("Guarded controller start requires Idle; automatic recovery disabled");
    }
  }
...
void Panda::_runController(TorqueCallback &control_callback) {
  try {
    ...
    last_error_ = std::make_shared<franka::Exception>(e.what());
  } catch (...) {
    last_error_ = std::make_shared<franka::Exception>("Unknown control thread exception");
  }
  // Publish only after last_error_ is ready. The staged runner reads errors
  // after this acquire/release handshake and joins with the Python GIL freed.
  // Never wait for Python logging while ending the control thread.
  control_thread_active_.store(false, std::memory_order_release);
}
...
bool Panda::moveToJointPositionConfigured(
    std::vector<Vector7d> waypoints, double speed_factor, double max_deviation,
    double planning_timeout, const Vector7d &stiffness, const Vector7d &damping,
    double dq_threshold, double success_threshold) {
  // Opt-in replay API: explicit planner parameters and no automatic recovery.
  // The legacy API above is unchanged for unrelated users.
  if (waypoints.empty() || !std::isfinite(speed_factor) || speed_factor <= 0 ||
      speed_factor > 0.03 || max_deviation != 0.0 ||
      !std::isfinite(planning_timeout) || planning_timeout <= 0 || planning_timeout > 30 ||
      !stiffness.allFinite() || !damping.allFinite() ||
      (stiffness.array() < 0).any() || (damping.array() < 0).any() ||
      !std::isfinite(dq_threshold) || dq_threshold <= 0 ||
      !std::isfinite(success_threshold) || success_threshold <= 0 || success_threshold > 0.03) {
    throw std::invalid_argument("Invalid guarded native replay parameters");
  }
  for (const auto &q : waypoints) {
    if (!q.allFinite() || (q.array() < joint_limits_.lower.array()).any() ||
        (q.array() > joint_limits_.upper.array()).any()) {
      throw std::invalid_argument("Guarded native waypoint outside verified FR3 limits");
    }
  }
  stopController();
  const auto initial = robot_->readOnce();
  if (initial.current_errors || initial.robot_mode != franka::RobotMode::kIdle) {
    throw std::runtime_error("Guarded replay requires Idle with no errors; automatic recovery disabled");
  }
  _setState(initial);
  waypoints.insert(waypoints.begin(), getJointPositions());
  auto traj = std::make_shared<motion::JointTrajectory>(
      waypoints, speed_factor, max_deviation, planning_timeout);
  if (traj->getDuration() == 0.0) return true;
  auto ctrl = std::make_shared<controllers::JointTrajectory>(
      traj, stiffness, damping, dq_threshold);
  _startController(ctrl, false);
  auto cb = _createTorqueCallback();
  _runController(cb);
  raiseError();
  const auto final_state = robot_->readOnce();
  _setState(final_state);
  const Vector7d q = Eigen::Map<const Vector7d>(final_state.q.data());
  return !final_state.current_errors &&
