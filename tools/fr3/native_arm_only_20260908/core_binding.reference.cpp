...
PYBIND11_MODULE(_core, m) {
  ...
  // Expose the existing native controller without changing its control law.
  py::class_<controllers::JointTrajectory, JointPosition,
             std::shared_ptr<controllers::JointTrajectory>>(m, "NativeJointTrajectoryController")
      .def(py::init<std::shared_ptr<motion::JointTrajectory>, const Vector7d&,
                    const Vector7d&, double>(),
           py::arg("trajectory"), py::arg("stiffness"), py::arg("damping"),
           py::arg("dq_threshold") = 1e-3);
