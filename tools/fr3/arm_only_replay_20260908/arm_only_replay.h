#pragma once
// Separate arm-only controller: no gripper command/feedback API or fake sensor.
// The installed passive gripper must stay at the operator-declared width.
...
class ArmOnlyReplay : public JointPosition {
 public:
  ...
  ArmOnlyReplay(const std::vector<double>& times,
    ...
    for (const auto& c : coeff_)
      if (c.col(7).head<5>().norm() != 0. || c(5,7) != coeff_[0](5,7) || c(5,7)<0 || c(5,7)>.088739804924)
        throw std::invalid_argument("Arm-only plan requires a constant, declared passive gripper width");
  ...
  franka::Torques step(const franka::RobotState& state, franka::Duration& period) override {
    ...
    last_time_ = t;
    record(state, t);
    if (external_fault_.load()) fail(6, "External arm-only supervisor failed");
    ...
    if (t >= times_.back() &&
        (q-target.head<7>()).cwiseAbs().maxCoeff() < .0015 &&
        dq.cwiseAbs().maxCoeff() < .001) {
  ...
  const std::string name() override { return "Arm-only slow replay - passive gripper, no SDK"; }
 ...
 private:
  ...
  bool started_=false;
  double last_time_=0., track_bad_s_=0., tcp_bad_s_=0.;
  ...
  void record(const franka::RobotState& state, double time) {
    ...
    row[j++]=std::numeric_limits<double>::quiet_NaN(); // Gripper NOT MEASURED.
