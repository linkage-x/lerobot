...
#include "controllers/timed_replay_candidate.h"
#include "controllers/arm_only_replay.h"
...
PYBIND11_MODULE(_core, m) {
  ...
  m.attr("_FR3_LIMITS_PATCH") = "fr3-5.8.1-protocol9-20260907-candidate";
  m.attr("_FR3_CONFIGURED_NATIVE_API") = 1;
  m.attr("_FR3_TIMED_OFFLINE_CANDIDATE") = 1;
  m.def("joint_limits_for_server_version", [](uint16_t version) {
    const auto limits = robotJointLimitsForServerVersion(version);
    py::dict result;
    result["lower"] = limits.lower;
    result["upper"] = limits.upper;
  ...
  py::class_<Panda>(m, "Panda", R"delim(
     The main interface of panda-py to control the robot.
      ...
      .def("start_controller_guarded", &Panda::startControllerGuarded,
           py::call_guard<py::gil_scoped_release>())
      .def("get_joint_limits_lower", &Panda::getJointLimitsLower)
      .def("get_joint_limits_upper", &Panda::getJointLimitsUpper)
      .def("move_to_joint_position_configured", &Panda::moveToJointPositionConfigured,
           py::call_guard<py::gil_scoped_release>(), py::arg("waypoints"),
           py::arg("speed_factor"), py::arg("max_deviation"), py::arg("planning_timeout"),
           py::arg("stiffness"), py::arg("damping"), py::arg("dq_threshold"),
      ...
      .def("control_thread_active", &Panda::controlThreadActive)
      .def("stop_controller", &Panda::stopController, py::call_guard<py::gil_scoped_release>())
  ...
  py::class_<TimedReplayCandidate, JointPosition, std::shared_ptr<TimedReplayCandidate>>(m, "TimedReplayCandidate")
      .def(py::init<const std::vector<double>&, const std::vector<TimedReplayCandidate::Coeff>&,
                    const std::vector<Eigen::Matrix4d>&, const std::vector<Eigen::Vector3d>&,
                    const Eigen::Matrix4d&>())
      .def("evaluate", &TimedReplayCandidate::evaluate, py::arg("time"), py::arg("derivative")=0)
      .def("fk", &TimedReplayCandidate::fk)
      .def("duration", &TimedReplayCandidate::duration)
      .def("feed_gripper", &TimedReplayCandidate::feedGripper)
      .def("acknowledge_gripper_command", &TimedReplayCandidate::acknowledgeGripperCommand)
      .def("abort_external", &TimedReplayCandidate::abortExternal)
      .def("fault_code", &TimedReplayCandidate::faultCode)
      .def("completed", &TimedReplayCandidate::completed)
      .def("telemetry", &TimedReplayCandidate::telemetry)
      .def("_offline_start", [](TimedReplayCandidate& c, const Vector7d& q, const Vector7d& dq, const Eigen::Matrix4d& tcp) {
          franka::RobotState s{};
          std::copy(q.data(), q.data()+7, s.q.begin());
          std::copy(dq.data(), dq.data()+7, s.dq.begin());
          std::copy(tcp.data(), tcp.data()+16, s.O_T_EE.begin());
          c.start(s, nullptr);
        })
      .def("_offline_step", [](TimedReplayCandidate& c, double t, const Vector7d& q, const Vector7d& dq,
                               const Eigen::Matrix4d& tcp, uint64_t period_ms) {
          franka::RobotState s{};
          std::copy(q.data(), q.data()+7, s.q.begin());
          std::copy(dq.data(), dq.data()+7, s.dq.begin());
          std::copy(tcp.data(), tcp.data()+16, s.O_T_EE.begin());
          c.setTime(t);
          franka::Duration period(period_ms);
          const auto tau = c.step(s, period);
          py::dict out; out["torque"] = tau.tau_J; out["finished"] = tau.motion_finished;
          return out;
        })
      .def("_offline_set_time", [](TimedReplayCandidate& c, double t) { c.setTime(t); });

  py::class_<ArmOnlyReplay, JointPosition, std::shared_ptr<ArmOnlyReplay>>(m, "ArmOnlyReplay")
      .def(py::init<const std::vector<double>&, const std::vector<ArmOnlyReplay::Coeff>&,
                    const std::vector<Eigen::Matrix4d>&, const std::vector<Eigen::Vector3d>&,
                    const Eigen::Matrix4d&>())
      .def("evaluate", &ArmOnlyReplay::evaluate, py::arg("time"), py::arg("derivative")=0)
      .def("fk", &ArmOnlyReplay::fk)
      .def("duration", &ArmOnlyReplay::duration)
      .def("abort_external", &ArmOnlyReplay::abortExternal)
      .def("fault_code", &ArmOnlyReplay::faultCode)
      .def("completed", &ArmOnlyReplay::completed)
      .def("telemetry", &ArmOnlyReplay::telemetry)
      .def("_offline_start", [](ArmOnlyReplay& c, const Vector7d& q, const Vector7d& dq, const Eigen::Matrix4d& tcp) {
          franka::RobotState s{};
          std::copy(q.data(),q.data()+7,s.q.begin());std::copy(dq.data(),dq.data()+7,s.dq.begin());
          std::copy(tcp.data(),tcp.data()+16,s.O_T_EE.begin());c.start(s,nullptr);
        })
      .def("_offline_step", [](ArmOnlyReplay& c, double t, const Vector7d& q, const Vector7d& dq,
                               const Eigen::Matrix4d& tcp, uint64_t ms) {
          franka::RobotState s{};
          std::copy(q.data(),q.data()+7,s.q.begin());std::copy(dq.data(),dq.data()+7,s.dq.begin());
          std::copy(tcp.data(),tcp.data()+16,s.O_T_EE.begin());c.setTime(t);
          franka::Duration period(ms);const auto tau=c.step(s,period);
          py::dict out;out["torque"]=tau.tau_J;out["finished"]=tau.motion_finished;return out;
        });
  ...
  py::class_<PureTorque, TorqueController, std::shared_ptr<PureTorque>>(
      m, "PureTorque", R"delim(
          Pure torque controller without any filtering or damping.
          Directly passes through the desired torques to the robot.
          
          Warning: This controller provides no safety features like filtering
          or damping. Use with caution.
      )delim")
      .def(py::init<>())
      .def("set_control", &PureTorque::setControl,
           py::call_guard<py::gil_scoped_release>(), py::arg("torque"),
           "Set the desired joint torques directly (no processing)")
      .def("get_torques", &PureTorque::getTorques,
           py::call_guard<py::gil_scoped_release>(),
           "Get the current desired torques")
      .def_property_readonly("name", &PureTorque::name);
