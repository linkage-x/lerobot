...
class FreshBoxMeasurement:
    """Decode BoxClient snapshots without mistaking repeated cache reads for data.

    The first MCU timestamp is a baseline, not a fresh sample. A device reboot
    or timestamp wrap stops this candidate rather than silently resynchronizing.
    """
    def __init__(self, expected_device_id):
        self.device_id = int(expected_device_id)
        if self.device_id <= 0:
            raise ValueError("Explicit physical gripper device ID required")
        self.last_timestamp = None
        self.sequence = 0

    def decode(self, snapshot):
        status = snapshot.get("status", {})
        if not status.get("active") or int(status.get("device_id", -1)) != self.device_id:
            raise ValueError("Inactive or wrong physical BOX device")
        sensor = snapshot.get("sensors", {}).get("box_gripper", {})
        timestamp, width = sensor.get("timestamp"), sensor.get("distance_m")
        if not isinstance(timestamp, int) or isinstance(timestamp, bool) or timestamp <= 0:
            raise ValueError("Missing gripper MCU timestamp")
        width = float(width)
        if not np.isfinite(width) or not 0 <= width <= WIDTH_MAX + 1e-12:
            raise ValueError("Invalid metric gripper measurement")
        if self.last_timestamp is None:
            self.last_timestamp = timestamp
            return None
        if timestamp < self.last_timestamp:
            raise ValueError("Gripper timestamp moved backwards; reset/wrap needs explicit handling")
        if timestamp == self.last_timestamp:
            return None
        self.last_timestamp = timestamp
        self.sequence += 1
        return self.sequence, width


def ingest_gripper(controller, decoder, read_snapshot):
    try:
        result = decoder.decode(read_snapshot())
        if result is not None:
            seq, width = result
            controller.feed_gripper(seq, controller.get_time(), width)
        return result
    except Exception:
...
def dispatch_gripper(controller, scheduler, send_metric_width):
    """Transport-neutral wiring; True means confirmed submission, NOT arrival.

    Never feed the measurement watchdog from a command's cached distance return.
    Never send final/reset/open commands in a fault handler.
    """
    if controller.fault_code():
        raise RuntimeError("Latched controller fault; no further gripper commands")
    phase = controller.get_time()
    width = scheduler.command(phase)
    if width is None:
        return None
    try:
        if send_metric_width(width) is not True:
            raise RuntimeError("Gripper command submission was not confirmed")
        controller.acknowledge_gripper_command(phase, width)
    except Exception:
        controller.abort_external()
        raise
    return width
