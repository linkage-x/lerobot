...
def replay_episode(
    *,
    ...
    print(f"[DEBUG] 录制 gripper[0]: state={states[0][7]:.3f} action={actions[0][7]:.3f}")

    record_joints_0 = states[0, 15:22]
    ...
    print(f"[DEBUG] FK from state[0] joints (pika_gripper_ee): {record_FK_ee[:3, 3]}")

    record_FK_tcp = np.asarray(kin_tcp.forward_kinematics(record_joints_0), dtype=np.float64)
    print(f"[DEBUG] FK from state[0] joints (pika_task_tcp): {record_FK_tcp[:3, 3]}")

    T_B_E_recorded_0 = pose_from_xyzquat(states[0, :7])
    print(f"[DEBUG] state[0] xyzquat → T_B_E: pos={T_B_E_recorded_0[:3, 3]}")

    diff_ee = np.linalg.norm(T_B_E_recorded_0[:3, 3] - record_FK_ee[:3, 3])
    diff_tcp = np.linalg.norm(T_B_E_recorded_0[:3, 3] - record_FK_tcp[:3, 3])
    ...
    initialize_gripper_state(
        ...
        model,
        mj_data,
        gripper_command=float(states[0, _GRIPPER_STATE_INDEX]),
    ...
    previous_gripper_target = float(states[0, _GRIPPER_STATE_INDEX])
    ...
    print(f"\n[RESULT] 重播完成:")
    print(f"  平均位置误差: {np.mean(pos_errors_mm):.2f} mm")
    print(f"  最大位置误差: {np.max(pos_errors_mm):.2f} mm")
    print(f"  平均旋转误差: {np.mean(rot_errors_deg):.2f} deg")
    print(f"  最大旋转误差: {np.max(rot_errors_deg):.2f} deg")
