...
Parent <-> child use two ``multiprocessing.Queue``\\s, passed as args::
    ...
    cmd_q  parent -> child:
        ...
        ("start_episode_at", str(episode_dir), target_mono_ns,
         target_frame_index, frame_period_ns)
        ("stop_episode",)
        ("stop_episode_at", target_mono_ns, expected_frames,
         target_frame_index, frame_period_ns)
        ("disconnect",)
...
def _sample_pts_ns(first_sample: Any, gst_clock_time_none: int) -> int | None:
    try:
        ...
        if buf is None or buf.pts is None:
            return None
        pts = int(buf.pts)
        if pts >= int(gst_clock_time_none):
            return None
        return pts
    except Exception:
        return None
...
def _fragment_dict(
    ...
    """
    first_pts_ns = _sample_pts_ns(first_sample, gst_clock_time_none)
    first_pts_s = None if first_pts_ns is None else float(first_pts_ns) / 1e9
