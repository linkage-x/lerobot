...
Options:
  ...
  --pwm-chip NAME        PWM chip passed to setup_sync.sh (default: ${PWM_CHIP})
  --pwm-id N             PWM channel passed to setup_sync.sh (default: ${PWM_ID})
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --pwm-chip) PWM_CHIP="$2"; shift 2 ;;
    --pwm-id) PWM_ID="$2"; shift 2 ;;
...
probe_log_error_matches() {
  local log="$1"
  [[ -f "${log}" ]] || return 1
  grep -E \
    'NvBufSurfaceFromFd Failed|dmabuf_fd -1|Error generated\..*gstnvarguscamerasrc|Failed to create CaptureSession|Argus Error Status|Error turning on streaming|CONSUMER: ERROR OCCURRED|TIMEOUT|UNAVAILABLE|waitForIdle\(\) timed out' \
    "${log}" || true
}
...
probe_sids() {
  ...
  for sid in $(split_csv "${csv}"); do
    ...
    rc=0
    timeout "${PROBE_TIMEOUT_S}" gst-launch-1.0 -q \
      ...
      ! fakesink sync=false async=false >"${log}" 2>&1 || rc=$?

    probe_errors="$(probe_log_error_matches "${log}")"
    if [[ ${rc} -eq 0 && -z "${probe_errors}" ]]; then
      echo "PROBE_OK sid=${sid}"
      current_ok+=("${sid}")
    else
      if [[ ${rc} -eq 0 ]]; then
        echo "PROBE_FAIL sid=${sid} rc=0 reason=log-error"
      else
        echo "PROBE_FAIL sid=${sid} rc=${rc}"
      fi
      if [[ -n "${probe_errors}" ]]; then
        echo "Probe error signatures:"
        printf '%s\n' "${probe_errors}" | tail -n 20
      else
        tail -n 20 "${log}" || true
      fi
      current_fail+=("${sid}")
