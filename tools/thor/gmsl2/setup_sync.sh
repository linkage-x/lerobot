...
# `boost_clock.sh` to lock VI/ISP/NVCSI/EMC at max, (4) arms `pwmchip6/pwm0`
...
echo "==> Programming PWM @ ${FPS}Hz on ${PWM_CHIP}/pwm${PWM_ID} (sync clock)"
PERIOD_NS=$(awk -v fps="$FPS" 'BEGIN { printf "%d", 1000000000 / fps }')
DUTY_NS=$(( PERIOD_NS / 2 ))
PWM=/sys/class/pwm/${PWM_CHIP}
PWM_CH=${PWM}/pwm${PWM_ID}
if [[ -d "$PWM" ]]; then
  if [[ -w "$PWM/unexport" ]]; then
    echo "${PWM_ID}" > "$PWM/unexport" 2>/dev/null || true
  fi
  if [[ ! -d "$PWM_CH" ]]; then
    echo "${PWM_ID}" > "$PWM/export"
  fi
  if [[ $DRY_RUN -eq 0 ]]; then
    ...
    echo "+ would set period=$PERIOD_NS duty=$DUTY_NS on $PWM_CH"
  fi
else
