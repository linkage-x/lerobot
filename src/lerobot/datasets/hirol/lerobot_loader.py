...
class LerobotLoader(DataLoaderBase):
    def __init__(self, config, task_dir, json_file_name = "data.json", action_type = ActionType.JointPosition,
        ...
        self._video_backend = config.get("video_backend", None)
        # 从配置读取抽帧步长，默认 3；兼容两种键名
        self._skip_steps = int(config.get("skip_steps_nums", config.get("skip_steps", 3)))
    ...
    def get_example_feature_dim(self, example_step):
        ...
        for key, image in images.items():
            ...
            if hasattr(image, "shape"):
                ...
                # Lazily read only for size; avoid heavy decode later
                try:
                    with PILImage.open(image) as img:
                        h, w = img.height, img.width
                        # Use bands to infer channels; default to 3
                        bands = img.getbands() or ("R", "G", "B")
                        c = len(bands)
                        shape = (h, w, c)
                except Exception as e:
                    raise FileNotFoundError(f"Failed to read image to infer shape: {image} ({e})")
            else:
            ...
            for key, image in depths.items():
                ...
                if hasattr(image, "shape"):
                    ...
                    try:
                        with PILImage.open(image) as img:
                            h, w = img.height, img.width
                            # Depth images are often single-channel; default to 1
                            bands = img.getbands() or ("Y",)
                            c = len(bands)
                            shape = (h, w, c)
                    except Exception as e:
                        raise FileNotFoundError(f"Failed to read depth image to infer shape: {image} ({e})")
                else:
    ...
    def convert_dataset(self):
        ...
        episode_dir = sorted(os.listdir(task_dir))[0]
        example_episode, _ = self.load_episode(task_dir, episode_dir, self._skip_steps)
        ...
        # 单一 task 下的所有 episodes
        dirs = sorted(os.listdir(task_dir))
        for cur_episode_dir in tqdm(dirs, desc=f"processing episodes", unit="episode"):
            episode_data, text_info = self.load_episode(task_dir, cur_episode_dir, self._skip_steps)
            ...
            for num_step, step in tqdm(enumerate(episode_data), desc=f"processing steps", unit="step"):
                ...
                for idx, image_key in enumerate(self._image_keys):
                    ...
                    img_val = step[source][image_key]
                    # If the reader provided a file path, open it into a PIL.Image
                    if isinstance(img_val, str):
                        try:
                            img_obj = PILImage.open(img_val)
                            # For color streams, ensure RGB; for depths keep mode
                            if source == "colors" and img_obj.mode != "RGB":
                                img_obj = img_obj.convert("RGB")
                            frame_feature[f"observation.images.{image_key}"] = img_obj
                        except Exception as e:
                            raise FileNotFoundError(f"Failed to open image '{img_val}' for key '{image_key}': {e}")
                    else:
                        frame_feature[f"observation.images.{image_key}"] = img_val
                # obs states
                state_list = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str,
                        default=os.path.join(cur_path, 'config', 'insert_pinboard.yaml'),
    ...
    # 单一 task 路径从 data_dir 读取
    task_dir = config.get("data_dir")
    if task_dir is None:
        raise ValueError("data_dir is required in config YAML for single task mode")
