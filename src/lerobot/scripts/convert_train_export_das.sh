...
echo "[1/3] 转换 MCAP 为 LeRobot 数据集..."

# 校验 DATA_CFG 的 output.task_dir
if ! python - <<'PY'
import os, sys, yaml
cfg_path = sys.argv[1]
with open(cfg_path, 'r', encoding='utf-8') as f:
    dc = yaml.safe_load(f)
out = (dc or {}).get('output') or {}
td = out.get('task_dir')
if not td:
    print('错误: DATA_CFG.output.task_dir 未设置', file=sys.stderr)
    sys.exit(2)
td_exp = os.path.expanduser(td)
root = td_exp if os.path.isabs(td_exp) else os.path.abspath(td_exp)
if os.path.exists(root):
    print(f"错误: 输出目录已存在: {root}", file=sys.stderr)
    sys.exit(3)
print(root)
PY
"$DATA_CFG"; then
  echo "请设置唯一且可写的 output.task_dir (且该目录必须不存在)。" >&2
  exit 1
fi

echo "+ python -m src.lerobot.datasets.hirol.das2lerobot_converter -c \"$DATA_CFG\""
python -m src.lerobot.datasets.hirol.das2lerobot_converter -c "$DATA_CFG"
