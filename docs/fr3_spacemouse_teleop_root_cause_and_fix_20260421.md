...
在上述基础上，又对 `FR3MujocoEnv` 实施了第一版关节级重力补偿：

- 保留现有 MuJoCo joint position actuator
- 在每个物理步前，向 arm 的 `qfrc_applied` 注入“零速度状态下计算的 bias 力矩”
- 目的不是构造完整动力学控制器，而是先抵消主要的重力载荷，减少 `-z` 下沉和由其耦合出的 `-x` 偏移

同时，为避免过补偿，补偿比例没有取 `1.0`，而是取了当前更平衡的默认值：

- `enable_arm_gravity_compensation=True`
- `arm_gravity_compensation_scale=0.5`

与此前的默认 teleop 配置组合后，当前默认配置为：
...
1. 纯 `+x`
- 真实 TCP 累计位移约 `[+0.009761, -0.000003, -0.000417] m`
- 位置误差约 `[+0.000230, +0.000003, +0.000398] m`
- 姿态误差约 `0.0558 deg`

2. 纯 `+z`
- 真实 TCP 累计位移约 `[-0.000230, -0.000003, +0.009581] m`
- 位置误差约 `[+0.000221, +0.000003, +0.000401] m`
- 姿态误差约 `0.0571 deg`

这意味着：

- `-x/-z` 现象没有被数学上“消灭”
- 但已经从“目标正向、真实 TCP 反向漂移且明显低头”收敛到“约 `0.4 mm / 0.056 deg` 量级的执行层误差”
- 从工程视角看，当前阶段已经完成了可观的止血和收敛

## 后续优先级

下一步如果还要继续逼近“几乎看不见残差”，正确方向不是再回去修改 SpaceMouse 映射，而是继续沿执行层升级：

1. 第二版控制器：joint PD + gravity compensation
2. 更进一步：任务空间控制 / 更完整的动力学前馈

但这两项 **当前优先级下调**。

原因：

- 当前问题已经从“错误方向和明显低头”收敛为较小跟踪误差
- 继续压榨剩余误差的成本明显高于当前收益
- 后续如需继续优化，应作为独立的小专题处理，而不是当前主线阻塞项

因此，本阶段建议结论为：

- 当前修复先到此为止
- 第一版重力补偿作为已验证方案保留
- 第二版控制器作为未来低优先级改进项，后续再做
