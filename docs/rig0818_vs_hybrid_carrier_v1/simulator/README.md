...
The `*_report.json` files are **not tracked**: they are deterministic output of
the runners below, rebuilt from the frozen `inputs/` (which are tracked, because
they came off the rig and cannot be regenerated). Until you rebuild them the
page says so and names the command. The numbers that were read off them live in
`RESULTS.md`.
