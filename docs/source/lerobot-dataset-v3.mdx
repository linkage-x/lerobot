...
> [!IMPORTANT]
> The canonical `timestamp` must stay on the nominal dataset clock: `frame_index / fps`.
> If you also need real wall-clock or sensor capture time, store it in a separate custom feature instead of overloading `timestamp`.
> LeRobot enforces this invariant when writing frames and will reject off-grid `timestamp` overrides.
> Mixing wall-clock `timestamp` values with videos encoded at the declared `fps` will make image playback, scalar timelines, `delta_timestamps`, and replay drift out of sync.
