...
def test_pipeline_desc_with_recorder_preview_defers_preview_branch():
    ...
    assert "splitmuxsink name=mux_7" in desc
    # The tee sits on the raw pre-encoder NVMM branch (2026-06-03 fix): the
    # preview branch must not spin up an extra H26x decoder per camera, so it
    # attaches before nvv4l2h265enc, not after h265parse.
    assert desc.index("tee name=t_7") < desc.index("nvv4l2h265enc")
